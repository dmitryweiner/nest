...
type
  TForm1 = class(TForm)
    ...
    procedure Image2MouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
      X, Y: integer);
    procedure Image2MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
      X, Y: integer);
    procedure Image2MouseMove(Sender: TObject; Shift: TShiftState; X, Y: integer);
    procedure RxTrayIcon1Click(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
      X, Y: integer);
  private
...
type
  nestpoint = record
    ...
    Next: pointer;
    //ссылка на следующее гнездо (оставлена для совместимости)
    ...
    R_by_days: TList;
    //ср. расст. до ближайших соседей по дням (массив объектов типа R_and_day)
...
function CalcAv_date(day: integer; list_in: TList): string;
  //вычисление среднего расстояния до ближайших соседей
var
  p1: ^nestpoint;
  i, i_max: integer;
  max: real;
...
begin
  ...
  begin
    ...
    if k < 0 then
      ...
      mash := K * (Image1.Width) / (-min_y + max_y);
    cx := (max_x + min_x) / 2;
    cy := (max_y + min_y) / 2;
    //вывод
    t := first_p;
    ...
    begin
      ...
      begin
        ...
        Image1.Canvas.Rectangle(Image1.Width div 2 + round((p1^.x - cx) * mash) - 1, Image1.Height div
          2 - round((p1^.y - cy) * mash) - 1, Image1.Width div 2 + round(
          (p1^.x - cx) * mash) + 1, Image1.Height div 2 - round((p1^.y - cy) * mash) + 1);
        Image1.Canvas.Font.Color := clRed;
        Image1.Canvas.TextOut(Image1.Width div 2 + round((p1^.x - cx) * mash), Image1.Height div
          2 - round((p1^.y - cy) * mash), s1);
      end
      else
      begin
...
y0:=cy_m;
    ...
    begin
      ...
      begin
        ...
        Canvas.Rectangle(cx_m + round((p1^.x - cx) * mash) - 1, cy_m - round(
          (p1^.y - cy) * mash) - 1, cx_m + round((p1^.x - cx) * mash) + 1, cy_m - round((p1^.y - cy) * mash) + 1);
      ...
      begin
        ...
        Canvas.Rectangle(cx_m + round((p1^.x - cx) * mash) - 1, cy_m - round(
          (p1^.y - cy) * mash) - 1, cx_m + round((p1^.x - cx) * mash) + 1, cy_m - round((p1^.y - cy) * mash) + 1);
        Canvas.Ellipse(cx_m + round((p1^.x - p1^.R_av - cx) * mash), cy_m - round(
          (p1^.y + p1^.R_av - cy) * mash), cx_m + round((p1^.x + p1^.R_av - cx) * mash), cy_m -
          round((p1^.y - p1^.R_av - cy) * mash));
...
begin
  ...
  begin
    ...
    case Form1.RGtype.ItemIndex of
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            s_to_ListBox := s_to_ListBox + Integer_to_str(temp_p^.Neib_Count) + ' ';
            //сколько соседей
        ...
        StatusBar1.SimpleText :=
          'Данные загружены. Предварительная обработка проведена.';
        L.Clear;
        //TB1.Enabled:=True;
      end;
      2:
      begin
        ...
        begin
          ...
          temp_p := p;
          while temp_p <> nil do
            //перегружаем в TList гнёзда с нужной датой
          begin
          ...
          begin
            ...
            SelectNeibs(L);
            CalcAv_date(i, L);
            //расчет средних расстояний. Средние расстояния сохраняются в глобальном списке
          end;
          L.Clear;
        end;//for
    ...
    DrawMap(1, i0, j0);
    StatusBar1.SimpleText :=
      'Вычисления завершены. Результат сохранен в буфер обмена.';
  end
  else
  begin
...
begin
  ...
  DrawMapR(-1);
  StatusBar1.SimpleText :=
    'Cоседи отобраны. Нажмите "Вычислить средние расстояния" для продолжения работы.';
end;
...
procedure TForm1.Image2MouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState;
  X, Y: integer);
begin
  i2 := x;
  j2 := y;
